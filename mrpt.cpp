...
void MultireweightHistosPT::internalHistogramReweighter(double targetBeta, Double2Array& histograms, unsigned index) {
    LogVal arguments[binCount];
    arguments[0] = lOmega_m[0] * toLogValExp(-targetBeta * U_m[0]);
    LogVal normalization = arguments[0];
    for (unsigned m = 1; m < binCount; ++m) {
        arguments[m] = lOmega_m[m] * toLogValExp(-targetBeta * U_m[m]);
    ...
    for (unsigned m = 0; m < binCount; ++m) {
        arguments[m] /= normalization;
        histograms[index][m] = toDouble(arguments[m]);
    }
}

double MultireweightHistosPT::internalHistogramOverlap(const Double2Array& histograms, unsigned index1, unsigned index2) {
    double overlap = 0.0;
    for (unsigned m = 0; m < binCount; ++m) {
        overlap += min(histograms[index1][m], histograms[index2][m]);
    }
    return overlap;
}

double MultireweightHistosPT::internalAverageNeighborOverlap(const BetaRange& betas) {
    unsigned nb = betas.getSize();
    Double2Array reweightedHistograms(boost::extents[nb][binCount]);
    for (unsigned l = 0; l < nb; ++l) {
        internalHistogramReweighter(betas[l], reweightedHistograms, l);
    }
    double summedOverlap = 0;
    for (unsigned l = 0; l < nb - 1; ++l) {
        summedOverlap += internalHistogramOverlap(reweightedHistograms, l, l + 1);
    }
    return summedOverlap / (nb - 1);
}

//Iteratively reweights the histogram with index active until its overlap with the one at index passive
//is equal to targetOverlap within accuracy, thereby make sure not to cross index limit
//active can be passive + 1 (=> limit < active), or passive - 1 (=> limit > active)
void MultireweightHistosPT::internalReweightHistogramToOverlap(Double2Array& curHistograms, BetaRange& curBetas,
        unsigned active, unsigned passive, unsigned limit, double targetOverlap, double accuracy) {
    double b0 = curBetas[passive];
    double bLimit = curBetas[limit];
    double bMin, bMax;
    int awayDirection;              //+1 if active is at a higher beta, -1 if passive is at a higher beta
    if (b0 < bLimit) {
        bMin = b0;
        bMax = bLimit;
        awayDirection = +1;
    ...
    double b1 = (bMax + bMin) / 2;
    internalHistogramReweighter(b1, curHistograms, active);

    //reweight until the desired overlap is achieved
    double ov = 0;
    bool finished = false;
    while (not finished) {
        if ((bMax - bMin) / bMax < 1E-3) {
//          cout << "equal!" << endl;
            finished = true;
        } else {
            //overlap between passive and active histogram
            ov = internalHistogramOverlap(curHistograms, passive, active);
            double deltaOv = (ov - targetOverlap) / targetOverlap;
            if (deltaOv > accuracy) {
                //overlap too big, move b1 away
                switch (awayDirection) {
                case +1:
                    bMin = b1;
                    b1 = bMax - .5 * (bMax - b1);
                    break;
            ...
            } else if (deltaOv < -accuracy) {
                //overlap too little, move b1 closer
                switch (awayDirection) {
                case +1:
                    bMax = b1;
                    b1 = bMin + .5 * (b1 - bMin);
...
BetaRange MultireweightHistosPT::reweightToConstantEnergyOverlap(double betaMin, double betaMax, unsigned targetNumBetas, unsigned maxIterations, double accuracy) {
    out << "Starting iterative histogram reweighting to achieve constant overlap with " << targetNumBetas << " inverse temperatures from " << betaMin << " to " << betaMax << "..." << endl;

    //use the old average overlap as a starting value for our target overlap
    double targetOverlap = internalAverageNeighborOverlap(betas);

    BetaRange newBetas(targetNumBetas);
    newBetas[0] = betaMin;
    newBetas[targetNumBetas - 1] = betaMax;

    Double2Array reweightedHistograms(boost::extents[targetNumBetas][binCount]);
    internalHistogramReweighter(betaMin, reweightedHistograms, 0);
    internalHistogramReweighter(betaMax, reweightedHistograms, targetNumBetas - 1);

    BetaRange lastIterationBetas(newBetas);
    ...
    while (stilladjusting) {
        ...
        while (continueIterating) {
            ...
            int lowIndex = 0;
            int highIndex = targetNumBetas - 1;
            while (lowIndex < highIndex) {
                //first try to shift the histogram at lowIndex + 1,
                //then that at highIndex - 1:
                internalReweightHistogramToOverlap(reweightedHistograms, newBetas,
                        lowIndex + 1,       //active
                        lowIndex,           //passive
                        highIndex,          //limit
                        targetOverlap, accuracy);
                internalReweightHistogramToOverlap(reweightedHistograms, newBetas,
                        highIndex - 1,      //active
                        highIndex,          //passive
                        lowIndex,           //limit
            ...
            //if no beta has changed more than accuracy break --> set new targetOverlap if necessary
            continueIterating = false;
            for (unsigned k = 0; k < targetNumBetas; ++k) {
                if (pow((newBetas[k] - lastIterationBetas[k]) / newBetas[k], 2)) {
                    //TODO: this has no effect
                    continueIterating = true;
                }
                lastIterationBetas[k] = newBetas[k];
            }

            //print some status
            out << "Target overlap: " << targetOverlap << endl;
            out << "betas: ";
            for (unsigned k = 0; k < targetNumBetas; ++k) {
                out << newBetas[k] << " ~ ";
            }
            out << "\noverlaps: ";
            for (unsigned k = 0; k < targetNumBetas - 1; ++k) {
                out << internalHistogramOverlap(reweightedHistograms, k, k + 1) << " ~ ";
            }
            out << endl;
        }
        //if any overlap is "much" smaller than the target overlap, re-adjust target overlap,
        //else -- we're done!
        stilladjusting = false;
        for (unsigned k = 0; k < targetNumBetas - 1; ++k) {
            double ov = internalHistogramOverlap(reweightedHistograms, k, k + 1);
            double delta = (ov - targetOverlap) / targetOverlap;
            if (delta < -accuracy) {
                stilladjusting = true;
                targetOverlap += (1.0 / (targetNumBetas - 1)) * (ov - targetOverlap);   //move targetOverlap closer to the diverging overlap
//              break;
            }
        }
    }
