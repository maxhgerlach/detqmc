...
std::tuple<bool,bool,DetModelLoggingParams,ModelParamsDetSDW,DetQMCParams> configureSimulation(int argc, char **argv) {
    ...
    loggingOptions.add_options()
        ...
        ("checkAndLogDetRatio", po::value<bool>(&loggingpar.checkAndLogDetRatio)->default_value(false),
         "verify correctness of local spin update transition probabilities")
        ("checkAndLogGreen", po::value<bool>(&loggingpar.checkAndLogGreen)->default_value(false),
         "verify correctness of updated Green's functions in local spin update")
        ("logGreenConsistency", po::value<bool>(&loggingpar.logGreenConsistency)->default_value(false),
         "check green consistency between wrapping / advancing, log differences")
        ("checkCheckerboardConsistency", po::value<bool>(&loggingpar.checkCheckerboardConsistency)->default_value(false),
         "Compare results obtained by multiplying dense / sparse hopping matrix exponentials. This just prints to stdout.")
        ;
