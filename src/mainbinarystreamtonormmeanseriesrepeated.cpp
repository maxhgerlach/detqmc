...
// Convert a sdw model binary stream to a normMeanPhi time series in
// stdout.  If saveConfigurationStreamInterval > 1 was used, repeat
// each read value that often in the output.
...
int main(int argc, char *argv[])
{
    ...
    uint32_t scsi = fromString<uint32_t>(meta.at("saveConfigurationStreamInterval"));
    ...
    while (binary_float_input) {
        ...
        if (binary_float_input) {
            ...
            for (uint32_t repeat = 0; repeat < scsi; ++repeat) {
                std::cout << norm << "\n";
            }
        }
    }
