...
// Using multiple histogram reweighting estimate the crossing
// temperature of the Binder cumulant or some other function of an
// observable at two different lattice sizes
...
#include "mrpt-jk.h"
#include "mrpt.h"
#include "mrpt-find-intersect.h"

using namespace std;

enum Quantity {
    BinderRatio,           // <|m|^4> / <|m|^2>^2
    ScaledKTSusceptibility // systemSize * <|m|^2> / L^(2-eta) with eta = 0.25
};
...
std::string quantity_string(Quantity quantity) {
    if (quantity == BinderRatio) {
        return "BinderRatio";
    }
    else if (quantity == ScaledKTSusceptibility) {
        return "ScaledKTSusceptibility";
    }
    else {
        return "";
    }
...
void findIntersection(Quantity quantity) {
    std::string qu_str = quantity_string(quantity);
    ...
    bool ok = true;
    cout << "Searching for intersection of " << qu_str << " between "
         << cpMin << " and " << cpMax;
    ...
    if (quantity == BinderRatio) {
        ...
        if (not use_jackknife) {
            cout << " without error bars." << endl;
            findScaledKTSusceptibilityIntersect(cp, ok, mr1, mr2, cpMin, cpMax);
        } else {
            cout << " with jackknife error bars." << endl;
            findScaledKTSusceptibilityIntersectError(cp, cpError, ok,
                                                     std::dynamic_pointer_cast<MultireweightHistosPTJK>(mr1),
                                                     std::dynamic_pointer_cast<MultireweightHistosPTJK>(mr2),
                                                     cpMin, cpMax, jackknifeBlocks);
        }
    } else {
        throw_GeneralError("quantity not understood");
    }
    ...
    string comments = "Estimated intersection point of " + qu_str + " for "  +
    ...
    if (quantity == BinderRatio) {
        ...
        writeOnlyMetaData(outputDirPrefix + "mrpt-scaledktsusceptibility-intersect-l"+L1+"l"+L2+".dat", meta,
                          comments);
    }
}
...
void initFromCommandLine(int argc, char** argv) {
    ...
    parser.add_option("quantity", "quantity for which we search the intersection: \"BinderRatio\" or \"ScaledKTSusceptibility\"", 1);
    ...
    parser.add_option("loadz1", "load partition function data for L1 from the indicated file", 1);
    parser.add_option("savez1", "save partition function data for L1 to the indicated file", 1);
    parser.add_option("loadz2", "load partition function data for L2 from the indicated file", 1);
    parser.add_option("savez2", "save partition function data for L2 to the indicated file", 1);
    ...
    parser.add_option("cp-range", "Takes two arguments determining the range of inverse temperatures between which to search for the intersection", 2);
    ...
    const char* one_time_opts[] = {"info1", "info2", "loadz1", "savez1", "loadz2", "savez2", "b", "j", "i", "sub-sample", "sort", "quantity"};
    ...
    if (parser.option("help")) {
        ...
        cout << "used to determine the crossing points of the Binder cumulants or scaled susceptiblities for the KT transition at two different lattice sizes." << endl;
    ...
    const clp::option_type& qu_arg = parser.option("quantity");
    if (qu_arg.argument() == "BinderRatio") {
        qu = BinderRatio;
    } else if (qu_arg.argument() == "ScaledKTSusceptibility") {
        qu = ScaledKTSusceptibility;
    } else {
        throw_GeneralError("quantity " + qu_arg.argument() + " not understood");
    }   
