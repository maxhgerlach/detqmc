...
import argparse
from scripthelpers import getHeader, parseHeader, is_file_older_than_a_day, is_file_more_than_a_day_older_than_file
...
parser.add_argument("--options", type=str, default="-j 20 --notau",
                    help="options to be passed to deteval, default: -j 20 --notau")
...
parser.add_argument("--evalOnlyNecessary", dest='evalOnlyNecessary', action='store_true', help="skip already evaluated subdirectories, if the last evaluation result is not at least 24 hours older than the data")
parser.add_argument("--no-evalOnlyNecessary", dest='evalOnlyNecessary', action='store_false', help="do not skip already evaluated subdirectories, if the last evaluation result is not at least 24 hours older than the data")
...
def eval_subdir(sd):
    ...
    path_eval_results = os.path.join(sd, "eval-results.values")
    path_info_dat = os.path.join(sd, "info.dat")
    if not glob(sd + "/*.series"):
        print( "=> no time series found: skip" )
        sys.stdout.flush()
        return sd
    elif evalOnlyRecent and os.path.exists(path_eval_results) and is_file_older_than_a_day(path_info_dat):
        print( "=> already evaluated and data is older than 24 hours: skip evaluation, but take into account" )
        sys.stdout.flush()
        return ''
    elif evalOnlyNew and os.path.exists(path_eval_results):
        print( "=> already evaluated: skip evaluation, but take into account" )
        sys.stdout.flush()
        return ''
    elif evalOnlyNecessary and os.path.exists(path_eval_results) and not is_file_more_than_a_day_older_than_file(path_eval_results, path_info_dat):
        ...
        print( "=> evaluate" )
        sys.stdout.flush()
        deteval_commandline = "deteval " + options
        pipes = subprocess.Popen(deteval_commandline, shell=True,
                                 stdout=subprocess.PIPE, stderr=subprocess.PIPE )
        std_out, std_err = pipes.communicate()
        print( std_out )
        if std_err:
            print( "Failure in deteval", file=sys.stderr )
            print( "commandline was: " + deteval_commandline, file=sys.stderr )
            print( std_err, file=sys.stderr )
            print( file=sys.stderr )
            sys.stderr.flush()

        if not os.path.exists("eval-results.values"):
            print( "=> did not write eval-results.values, skip after all" )
            sys.stdout.flush()
            skip_if_sd = sd
        else:
            skip_if_sd = ''
...
metadata = {sd: addControlParameterCount(parseHeader(getHeader(sd + "/eval-results.values"))) for sd in subdirs}
