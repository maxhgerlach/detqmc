...
def find_intersection_for_subdirs(tuple_list):
    """if possible call mrpt-find-intersect for subdirs in tuple_list,
    ...
    # execute mrpt-find-intersect calls
    for L1, L2, cpMin, cpMax in L1_L2_cpMin_cpMax:
        ...
        for quantity in ["BinderRatio", "ScaledKTSusceptibility"]:
            print "Finding", quantity, "intersection for L1=", L1, ", L2=", L2
...
# do two collection runs, one for each quantity
for quantity, pattern, outfstart in zip(
        ["BinderRatio", "ScaledKTSusceptibility"],
        ["/mrpt*binder*intersect*.dat",
         "/mrpt*scaledktsusceptibility*intersect*.dat"],
        ["mrpt-binderratio-intersect_", "mrpt-scaledktsusceptibility-intersect_"]):
    ...
    for tup in multivalValueTuples:
        ...
        ## find all assembled quantity crossing data
        ...
        for sd in relsubdirs:
            ...
            mrpt_intersect_files = glob(sd + pattern)
            ...
            for fname in mrpt_intersect_files:
                ...
                if controlParameterName in fdict:
                    ...
                    if controlParameterNameError in fdict:
                        ...
                        cpError = 0.0                    
                    mapToIntersectionResults.setdefault((L1,L2), {})[varvalue] = (
                        cp, cpError)
                else:
                    continue
        ...
        for L1,L2 in mapToIntersectionResults.keys():
            ...
            output_filename = prefix + outfstart + variable + "-" + \
            ...
            with open(output_filename, 'w') as output_file:
                ...
                output_file.write("## %s intersection\n" % quantity)
                output_file.write("## %s %s %s\n" % (variable, controlParameterName,
                                                     controlParameterNameError))
