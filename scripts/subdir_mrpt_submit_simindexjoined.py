...
# Run mrpt reweighting in each subdir, one job per subdir
# Note: One job takes like 30seconds, so this is not going to be to
# the admins' liking

# this version: restrict to subdirs with "simindexjoined" in name
...
if __name__ == '__main__':
    ...
    parser.add_argument("--prefix", type=str, default="",
                        help="only work on directories with this prefix")
    ...
    subdirs = [o for o in os.listdir(".") if os.path.isdir(o)
               and "simindexjoined" in o
               and (o.startswith(prefix) or o.startswith("./" + prefix))]
    ...
    for sd in subdirs:
        ...
        if jobname in runningJobs:
            ...
            continue
        elif not os.path.exists(sd + "/info.dat"):
            print "=> no info.dat found: skip"
            continue
