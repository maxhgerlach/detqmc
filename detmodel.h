...
DetModelGC<GC>::DetModelGC(const ModelParams& pars, unsigned greenComponentSize) :
	...
	if (timedisplaced) {
		...
		for_each_gc( [this](unsigned gc) {
			updateGreenFunctionUdV[gc] = [this, gc](unsigned targetSlice,
					                                const UdVnum& UdV_L, const UdVnum& UdV_R) {
...
template<unsigned GC>
std::vector<ScalarObservable> DetModelGC<GC>::getScalarObservables() {
	return obsScalar;
}

template<unsigned GC>
std::vector<VectorObservable> DetModelGC<GC>::getVectorObservables() {
	return obsVector;
}

template<unsigned GC>
std::vector<KeyValueObservable> DetModelGC<GC>::getKeyValueObservables() {
	return obsKeyValue;
}
...
template<unsigned GC>
void DetModelGC<GC>::setupUdVStorage() {
	auto setup = [this](unsigned gc) {
		...
		for (unsigned l = 1; l <= n - 1; ++l) {
			...
			UdVnum UdV_temp = udvDecompose<num>((B_lp1 * U_l) * arma::diagmat(d_l));
...
template<unsigned GC>
void DetModelGC<GC>::sweepSimple() {
...
template<unsigned GC>
MatNum DetModelGC<GC>::greenFromUdV(const UdVnum& UdV_l, const UdVnum& UdV_r) const {
	...
	UdVnum UdV_temp = udvDecompose<num>( inv(U_l * U_r) + diagmat(d_r) * (V_r * V_l) * diagmat(d_l) );
...
template<unsigned GC>
MatNum4 DetModelGC<GC>::greenFromUdV_timedisplaced(
...
template<unsigned GC>
void DetModelGC<GC>::advanceDownGreen(unsigned l, unsigned greenComponent) {
	...
	UdVnum UdV_L = udvDecompose<num>(arma::diagmat(d_l) * (V_l * B_l));
	UdV_L.U = U_l * UdV_L.U;
...
template<unsigned GC>
void DetModelGC<GC>::wrapDownGreen_timedisplaced(unsigned k, unsigned greenComponent) {
...
template<unsigned GC>
void DetModelGC<GC>::wrapDownGreen(unsigned k, unsigned greenComponent) {
	MatNum B_k = computeBmat[greenComponent](k, k - 1);
	green[greenComponent].slice(k - 1) = arma::inv(B_k) * green[greenComponent].slice(k) * B_k;
}
...
template<unsigned GC>
void DetModelGC<GC>::advanceUpGreen(unsigned l, unsigned greenComponent) {
	std::vector<UdVnum>& storage = UdVStorage[greenComponent];
...
template<unsigned GC>
void DetModelGC<GC>::advanceUpUpdateStorage(unsigned l, unsigned greenComponent) {
	std::vector<UdVnum>& storage = UdVStorage[greenComponent];
	...
	storage[l+1] = udvDecompose<num>(((B_lp1 * U_l) * arma::diagmat(d_l)));
	storage[l+1].V *= V_l;
};
...
template<unsigned GC>
void DetModelGC<GC>::wrapUpGreen_timedisplaced(unsigned k, unsigned greenComponent) {
...
template<unsigned GC>
void DetModelGC<GC>::wrapUpGreen(unsigned k, unsigned greenComponent) {
	MatNum B_kp1 = computeBmat[greenComponent](k + 1, k);
	green[greenComponent].slice(k + 1) = B_kp1 * green[greenComponent].slice(k) * arma::inv(B_kp1);
}

template<unsigned GC>
void DetModelGC<GC>::sweepUp() {
