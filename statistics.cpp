...
double average(const std::vector<double>* vec, int start, int end) {
    if (end==0) {
        end = vec->size();
    }
//  assert(end > start);
    ...
    for (int i = start; i < end; ++i) {
        avg += double((*vec)[i]) / (end-start);
    }
    return avg;
}

double average(const std::vector<int>* vec, int start, int end) {
    if (end==0) {
        end = vec->size();
    }
//  assert(end > start);
...
double sqAverage(const std::vector<double>* vec, int start, int end) {
    if (end==0) {
        end = vec->size();
    }
//  assert(end > start);
...
double variance(const std::vector<double>* numbers, double meanValue, int N) {
    if (N == 0) {
        N = numbers->size();
    }
//  assert(N > 0);
...
double variance(const std::vector<int>* numbers, double meanValue, int N) {
    if (N == 0) {
        N = numbers->size();
    }
//  assert(N > 0);
